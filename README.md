...
- **メモ機能**: ユーザーごとのメモ作成・管理、公開・非公開設定、タグ付け、マークダウン対応、画像添付
...
- **ストレージ**: Supabase Storage（画像添付機能用）
...
4. データベースのセットアップ
   ...
   - 画像添付機能を使用する場合は `/sql/scripts/storage/note_images_setup.sql` を実行

5. ストレージバケットの設定（画像添付機能用）
   - Supabaseダッシュボードの「Storage」タブで新しいバケット「note_images」を作成
   - パブリックアクセス：無効
   - アップロード保護：有効
   - RLSポリシーを適切に設定（詳細は `/sql/scripts/storage/note_images_setup.sql` のコメントを参照）
...
✅ メモ検索機能（タイトル、内容、タグによる検索）  
✅ メモのソート機能（更新日、作成日、タイトル順）  
✅ メモの画像添付機能（アップロード、表示、マークダウン埋め込み）  
...
function NoteImagesComponent({ noteId }) {
  const { 
    images, 
    uploading, 
    uploadImage, 
    deleteImage 
  } = useNoteImages(noteId);
