...
-- アバター関連フィールドの追加
ALTER TABLE public.user_profiles
ADD COLUMN avatar_type TEXT NOT NULL DEFAULT 'sample', -- 'sample'または'custom'
ADD COLUMN selected_avatar_id TEXT; -- サンプルアバターの場合、選択したアバターID

-- オンラインステータス関連フィールドの追加
ALTER TABLE public.user_profiles
ADD COLUMN online_status TEXT NOT NULL DEFAULT 'offline', -- 'online', 'offline', 'busy'など
ADD COLUMN last_active_at TIMESTAMP WITH TIME ZONE;

-- バリデーション用チェック制約の追加
ALTER TABLE public.user_profiles
ADD CONSTRAINT valid_username CHECK (username ~ '^[a-z0-9_]{3,20}$'), -- 小文字、数字、アンダースコアのみ、3-20文字
